...
var DEFAULT_SETTINGS = {
    ...
    // Add token settings
    addTokenAllow: false,
    addTokenText: "Click here to add.",
    addTokenMethod: "POST",
    addTokenURL: '',
    addTokenJS: '',
    afterAddTokenJS: '',

    // Add facebook style search
    faceStyle: false,
    faceStyleText: "Search this term",
    ...
    confirmAddToken: function(settings) { return '<a href="#" class="'+settings.classes.addToken+'"> '+settings.addTokenText+'</a>'},
...
if (settings.addTokenAllow) {
    ...
    if (settings.addTokenJS) {
      eval(settings.addTokenJS);
      eval(setTimeout(settings.afterAddTokenJS, 400));
    } else {
      ...
      if (newTag) {
      dropdown.find('p').text('Adicionado!');
      add_token(newTag);
...
function show_dropdown_min_chars () {
    if(settings.minCharsText) {
        dropdown.html("<p>"+settings.minCharsText+"</p>");
        show_dropdown();
    } else {
        hide_dropdown();
    }
...
function populate_dropdown (query, results) {
    if(results && results.length) {
        ...
        if(settings.faceStyle) {
          var this_li = "<li class='token-input-dropdown-item' style='text-align: center; font-weight: bold; padding: 8px 0;' onclick=\"$('#form_busca').submit()\">" + settings.faceStyleText + ": " + query + "</li>";
...
function do_search() {
    ...
    if(settings.faceStyle) { $("#busca").val(query); }
